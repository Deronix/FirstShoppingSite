...
if (isset($_COOKIE['user_key'])) {
    ...
    // Modified to get cart items instead of favorites
    $favorites = [];
    $favorites_sql = "SELECT urunler.id, urunler.name, urunler.price, urunler.image, urunler.image2, urunler.image3, urunler.description, 
                              urunler.information,urunler_hesap_cart.quantity, urunler.comments, urunler.answer, urunler.question
                      FROM urunler_hesap_cart
                      LEFT JOIN urunler ON urunler_hesap_cart.product_id = urunler.id
                      WHERE urunler_hesap_cart.user_id = ?";
    ...
    // ... [keep regular products and popular products queries the same]
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $user_logged_in) {
    // Modified to remove from cart instead of favorites
    if (isset($_POST['remove-from-favorites'])) {
        $productId = $_POST['product_id'];
        $userId = $_SESSION['user_id'];

        $sql = "DELETE FROM urunler_hesap_cart WHERE user_id = ? AND product_id = ?";
    ...
    // Modified to add to favorites instead of cart
    if (isset($_POST['add-to-cart'])) {
        $productId = $_POST['product_id'];
        $userId = $_SESSION['user_id'];

        $checkSql = "SELECT * FROM urunler_hesap_favorites WHERE user_id = ? AND product_id = ?";
        ...
        if ($result->num_rows == 0) {
            $sql = "INSERT INTO urunler_hesap_favorites (user_id, product_id, added_at) VALUES (?, ?, NOW())";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ii", $userId, $productId);
            $stmt->execute();
            $stmt->close();
...
<head>
    ...
    <script defer src="hesap.js"></script>
    <title>Hesap</title>
</head>
        ...
        <?php if ($user_logged_in): ?>
            ...
            <div class="shopping-form" id="shopping-site">
                                        ...
                                        <div class="product">
                                            ...
                                            <div class="info-group">
                                                <form method="POST">
                                                    ...
                                                    <button type="submit" name="add-to-cart">Add To Favorites</button>
                                                </form>
                                                <form method="POST">
                                                    <input type="hidden" name="product_id" value="<?php echo $item['id']; ?>">
                ...
                <div class="confirmcart">
                    <a href="./../SatinAl/sat.php">Confirm Cart</a>
                </div>
                <div class="space"></div>
